...
{:steps [{:name "resolve-current-pr"
         ...
         {:name "heal-checks"
          ...
          :prompt-string {:type :template
                          :text "{{report}}"
                          :vars {"report" {:from {:step "resolve-current-pr" :yield :text}}}}
          :context [{:type :source
                     :from :workflow-original}
                    {:type :source
                     :from {:step "resolve-current-pr" :yield :text}}]}]}
