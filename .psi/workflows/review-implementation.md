...
{:steps [{:name "implementation-review"
         ...
         {:name "code-shape-follow-up"
          ...
          :judge {:system-prompt "You are a workflow routing judge. Respond with exactly one word: REPEAT or DONE."
                  :prompt "Respond exactly with one word. Return REPEAT if this full cycle found any new actionable feedback to address after the implementation review and the code-shape review. Return DONE if no new actionable feedback was found in the cycle. Use the actor session context, including the review and follow-up results, to decide."}
