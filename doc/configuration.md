...
name is not reserved. `:clear` is reserved for `/session-profile clear` and is
not selectable. `/session-profiles` lists valid profiles with readable settings
and invalid profiles with terse reasons.
