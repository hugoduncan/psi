...
- For the slash-command surface offered to UIs, the backend is the single
  ...
  - the Emacs `psi-emacs-slash-command-specs` `defcustom` survives as a user
    override/supplement (default trimmed to the Emacs-only `/skill:` affordance,
    which is not a backend routing target); its entries are merged after the
    backend specs, so backend built-in descriptions win on any name collision.
