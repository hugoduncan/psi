(ns extensions.ramora
  "Ramora extension — injects the Ramora protocol into the system prompt.
...
(def ^:private prompt-contribution-id "ramora-protocol")
(def ^:private protocol-resource-path "extensions/ramora/protocol.txt")
...
(defn- read-protocol-resource [resource-path]
  (try
    (if-let [resource (io/resource resource-path)]
      ...
      (throw (ex-info (str "Missing ramora protocol resource: " resource-path)
                      {:extension :ramora
                       :resource-path resource-path})))
    (catch clojure.lang.ExceptionInfo e
      (throw e))
    (catch Exception e
      (throw (ex-info (str "Failed reading ramora protocol resource: " resource-path)
                      {:extension :ramora
                       :resource-path resource-path}
                      e)))))

(def ^:private ramora-lambda
...
(defn- prompt-content []
  (if (lambda-mode?)
    ramora-lambda
    (str engage-prefix ramora-lambda)))
