...
(def ^:private prompt-contribution-id "mementum-protocol")
(def ^:private protocol-resource-path "extensions/mementum/protocol.txt")
...
(defn- read-protocol-resource [resource-path]
  (try
    (if-let [resource (io/resource resource-path)]
      ...
      (throw (ex-info (str "Missing mementum protocol resource: " resource-path)
                      {:extension :mementum
                       :resource-path resource-path})))
    (catch clojure.lang.ExceptionInfo e
      (throw e))
    (catch Exception e
      (throw (ex-info (str "Failed reading mementum protocol resource: " resource-path)
                      {:extension :mementum
                       :resource-path resource-path}
                      e)))))

(def ^:private mementum-lambda
...
(defn- prompt-content []
  (if (lambda-mode?)
    mementum-lambda
    (str engage-prefix mementum-lambda)))
