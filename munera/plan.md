...
- `munera/open/211-emacs-retry-backoff-footer-visible/` — ensure active LLM/provider retry backoff triggers the existing footer retry message path in Emacs, so users visibly see retry wait text without manual refresh. Design-only.
