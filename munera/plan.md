...
- `munera/open/241-emit-only-focused-session-events/` — restrict RPC event emission to the connection's focused session; non-focused session-scoped events (deltas, tool events, session/footer updates) are suppressed since the Emacs UI re-requests full content on refocus. Cross-session events (`context/updated`, `ui/*`) stay ungated. Design-only.
