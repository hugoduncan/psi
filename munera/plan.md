...
- `munera/open/201-verify-scheduler-execution/` — **verification-only** audit of the scheduler subsystem (pure model + live timer/dispatch firing/delivery path) end-to-end. Adds verifying test coverage + a findings inventory; defects are reported as new remediation tasks, not fixed here. One-shot/volatile scope (no persistence/recurrence).
