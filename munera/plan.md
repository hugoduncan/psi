...
- `munera/open/207-workflow-session-defaults-inheritance-snapshot/` — workflow runs should snapshot the inheritable default session details from their parent at invoke time (top-level steps from the invoking session; nested workflows from their parent run), so later changes (invoking session switching model, or user/project default-model changes) do not affect an already-running delegated workflow. Currently `resolve-step-session-config` reads the live parent session each resolve; `create-run` stores `:parent-session-id` but no defaults snapshot. Design-only; open questions on which details to snapshot and nested-parent semantics.
