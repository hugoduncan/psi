...
- [ ] Preserve incomplete nested `<tool_call><function=...><parameter=...>` text inside an outer parameter value; the parser currently recovers the apparent inner call by stealing the outer parameter/function/tool-call close tags, e.g. `printf '<tool_call><function=literal><parameter=x>'`, which executes literal parameter text instead of preserving the outer `bash` call's command string.
