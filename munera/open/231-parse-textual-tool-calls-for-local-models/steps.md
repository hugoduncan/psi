...
- [ ] Recover a well-formed textual tool-call block even when surrounding assistant prose includes quote characters immediately before or after the block; the parser currently suppresses conversion based on adjacent quotes outside the exact `<tool_call>...</tool_call>` span, which violates the requirement to remove/execute exact parsed blocks while preserving surrounding non-tool text.
