...
- [ ] Preserve literal `<tool_call>` / `</tool_call>` text inside parameter values; the parser currently rejects any function body containing those substrings, which can make otherwise valid command text malformed instead of preserving parameter text.
