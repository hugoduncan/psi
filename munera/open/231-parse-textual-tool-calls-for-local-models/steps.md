...
- [ ] Preserve literal adjacent `<tool_call><function=...>` text inside parameter values; the parser currently rejects any function body containing that exact substring even when it appears inside a parsed parameter value, so otherwise valid command text such as `printf '<tool_call><function=literal>'` becomes malformed instead of preserving parameter text.
