...
- [ ] Bound or linearize textual tool-call parsing so capability-enabled local-model responses with many `<tool_call>`/`</tool_call>` markers cannot impose quadratic-plus normalization cost; add a stress/regression test for malformed many-marker output.
