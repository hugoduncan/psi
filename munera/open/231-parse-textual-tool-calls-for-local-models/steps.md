...
- [ ] Preserve complete nested well-formed `<tool_call>...</tool_call>` text inside parameter values as literal parameter text; the current candidate scan can reject the outer call and recover/execute the inner literal block instead, e.g. `printf '<tool_call><function=x><parameter=y>z</parameter></function></tool_call>'`, violating parameter preservation and mixed malformed/well-formed semantics.
