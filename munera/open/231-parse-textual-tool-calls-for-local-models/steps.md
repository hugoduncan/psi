...
- [ ] Treat an unindexed existing provider/content block whose id is already `TURN_ID/toolcall/N` as occupying index `N` instead of allocating an additional hidden position; a preceding unindexed `turn-id/toolcall/0` block followed by one recovered textual call should allocate the recovered call as `/1`, not skip to `/2`. Add a focused regression.
- [ ] Separate, revert, or explicitly document as pre-existing/prerequisite the unrelated changes present in this task diff (entity-resolution/review workflow artifacts, OpenAI `parallel_tool_calls`, and the write-tool description tweak), so the task implementation matches the textual-tool-call design scope.
