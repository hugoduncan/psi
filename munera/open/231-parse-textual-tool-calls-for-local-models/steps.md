...
- [ ] Do not recover and execute a nested well-formed `<tool_call>...</tool_call>` that appears inside the parameter text of an outer malformed `<tool_call>` block; for example, an outer block made malformed by duplicate parameters should be preserved as ordinary text rather than executing the nested literal block.
