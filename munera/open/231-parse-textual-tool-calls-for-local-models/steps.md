...
- [ ] Reject multiple `<function=...>` blocks inside one `<tool_call>` even when separated by whitespace or parameter-looking text; the parser currently accepts a second function block as literal parameter text in cases like `</function> <function=...>`, violating the exactly-one-function rule.
