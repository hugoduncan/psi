...
- [ ] Validate request/result shape at the higher session-owned realization edge for workflow child-session creation.
...
- [ ] Extend workflow attempt-session tests to prove exact forwarding of the supported create request surface and one-attempt-one-session invariants.
- [ ] Extend workflow judge tests to prove judge child-session creation uses the shared seam with explicit judge defaults and projected preload messages.
