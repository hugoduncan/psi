...
- [ ] TT1: Add workflow-loader prompt content-lock coverage for the autonomous persistence contract that is currently only implemented in prompt text: no human approval request, allowed `mementum/memories/{slug}.md` / `mementum/knowledge/{topic}.md` output shapes (including knowledge frontmatter and memory symbol/one-insight constraints), mementum commit conventions, and the no-op `Do not commit if nothing changed` path.
- [ ] TT2: Add workflow-loader prompt content-lock coverage for the final-summary/reporting contract: resolved task path plus standalone/lifecycle authorization mode, extracted memories/knowledge, updated/skipped duplicates, zero-extraction success, and preservation of any lifecycle/review outcome supplied via the dedicated `{{implementation_review_yield}}` section.
