...
- [ ] IR1: Label or otherwise structure the `task-lifecycle` extraction delegate's implementation-review yielded-text context so `{{original}}` prompt rendering unambiguously identifies the `review-task-implementation` yield as the sole `PASS_STATUS: REVIEW_COMPLETE` source authorizing open-task extraction; add/update workflow-loader tests to lock this prompt-visible source distinction and prevent arbitrary prior `:workflow-original` text from authorizing open-task extraction.
