...
- [ ] Reconcile `doc/workflow-ir.md` structured-output invalid-policy/default retry wording with the landed workflow-judge behavior: invalid structured-output judge turns retry by default up to the built-in judge retry limit, while unsupported structured output still fails immediately.
