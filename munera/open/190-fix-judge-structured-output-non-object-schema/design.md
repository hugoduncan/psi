...
`psi.ai.providers.openai.codex-structured-output/structured-output-result` is also part of the provider structured-output result surface for the preservation invariant. It already uses `parse-json-value` at its result helper and should test parse success from the parse result map rather than from payload truthiness, so JSON `null` remains a present `:payload nil`. No Codex extraction rewrite is expected unless implementation reveals drift, but tests may cover this surface to protect the shared invariant.
...
- `components/ai/src/psi/ai/providers/openai/codex_structured_output.clj` — `structured-output-result` is in scope for the provider structured-output result invariant; current code already uses `parse-json-value`, but implementation/tests must ensure parse success is not gated on payload truthiness and `:payload nil` is retained
