...
- [ ] Reshape session lifecycle tests (`session_state/init_test`) to prove authoritative prompt behavior from `:prompt-contribution-ids` + root-registry-backed reads rather than merely preserving stale copied `:prompt-contributions` compatibility vectors across new/resume/fork initialization.
