...
- [x] Shape ranked fallback model mutation so the first ranked candidate reuses the child session's initial concrete model and only later fallback candidates trigger `set-execution-session-model!`.
