...
2. **Preserve ordered ranked candidates for workflow query models**
   - extend the workflow resolution/execution path so a workflow session step with `:model-query` has access to the ordered `[:ranking :ranked]` candidates from `psi.ai.model-selection/resolve-selection`
   - keep explicit concrete models on the existing single-model path
...
4. **Surface exhaustion coherently**
   - when all candidates fail, return a terminal step failure that reflects ranked fallback exhaustion and keeps enough failure detail for diagnosis
