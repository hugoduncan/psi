...
- [x] Record the canonical fallback-worthy execution failure predicate in `implementation.md` and identify the runtime seam where it is applied, so `implementation.md` no longer contradicts `design.md`, `plan.md`, and `steps.md` about that decision being made before code changes.
