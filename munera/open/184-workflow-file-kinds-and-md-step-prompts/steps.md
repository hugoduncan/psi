...
- [ ] Broaden `workflow-migration-validation-test` from a curated-required-workflow subset to corpus-wide contract validation, so the checked-in workflow tree cannot silently drift into invalid mixed-kind collisions or malformed migrated artifacts outside the named sample set.
