...
- [ ] Change `compile-and-validate-workflow-definition` catch block: store `{:message (ex-message e) :data (ex-data e)}` as `:compile-error` (was bare string); update docstring
- [ ] Implement formatter: add `format-compilation-errors` in `psi.workflow-runtime.ir` (or `error-format.clj`); cover all semantic error types and Malli structural error rendering; accepts `compile-error` as map `{:message … :data …}` or nil
- [ ] Wire formatter into `compile-definition-to-ir!` in `core.clj`
- [ ] Update existing tests in `target_ir_compiler_test.clj` that compare `:compile-error` to a bare string — change to `(get-in result [:compile-error :message])`
