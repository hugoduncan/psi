...
- [ ] Add test for `format-structural-error` `"invalid value"` fallback: hand-craft a Malli error entry with no `:message` and no `:type` key; assert output contains `"invalid value"` and does not end with a bare `":"`
