...
- [x] Correct the `env:` API-key lifecycle claims in
...
- [x] Update the EOF-terminal `CHANGELOG.md` entry that still says DeepSeek's
...
- [x] Reflect the shipped DeepSeek custom-provider capability in both required
...
- [x] Route `emit-chat-completion-finish!` through
...
- [x] Split `request-support/resolve-api-key` so key resolution and missing-key
...
- [x] Extract Anthropic SSE event dispatch from `stream-anthropic` into a
...
- [x] Remove the remaining review chronology from
