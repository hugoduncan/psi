...
- [x] Reject provider action invocation maps with extra same-namespace but non-schema `:psi.ui.invocation/...` keys before exposing them through EQL, and add focused coverage for fail-closed provider-error semantics.
- [x] Reject provider action descriptors with extra same-namespace but non-schema `:psi.ui.action/...` keys before exposing them through EQL, and add focused coverage for fail-closed provider-error semantics.
