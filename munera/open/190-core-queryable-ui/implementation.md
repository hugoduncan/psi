...
No new actionable test-shaping feedback after re-reading task artifacts, UI capability/provider normalization tests, graph discovery tests, real extension API and nullable API query coverage, RPC/TUI lifecycle tests including exceptional TUI shutdown, Emacs ERT coverage for `psi-emacs-show-active`, extension docs, and the latest provider collection-shape follow-up. The current tests are behaviour-focused, deterministic, and cover the main contract boundaries without duplicating already-completed follow-ups.
