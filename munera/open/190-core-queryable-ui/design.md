...
`:psi.ui/diagnostic` is a first-class root-queryable/discoverable EQL output for provider-error cases only. It is nil or absent for normal available, missing-provider, no-attached-UI, and unsupported-capability results unless a future task adds broader UI diagnostics. When present, it is bounded serialisable text intended for debugging resolver/provider failures, not user-facing UI copy. The resolver must redact stack traces, frontend object printed forms, tokens, paths containing secrets, and arbitrary exception data by default; it may include a short provider-error class/category and message. The bounded length should be small enough for graph discovery/debug output (for example 512 characters or the nearest existing diagnostic-text bound). `:psi.ui/diagnostic` must be included in resolver output/index metadata so callers can explicitly request it, but extension guidance should treat it as optional troubleshooting data and branch on unavailable reason keywords instead.

Exact unsupported/headless/provider-error semantics:

- Missing provider: `:psi.ui/type nil`, `:psi.ui/available? false`, `:psi.ui/capabilities []`, `:psi.ui/actions []`; `:psi.ui/make-visible-action` returns an unavailable make-visible descriptor with reason `:psi.ui.unavailable.reason/no-provider`; `:psi.ui/diagnostic` is nil/absent.
- Provider reports no attached UI: `:psi.ui/type` may be nil or the known adapter type, `:psi.ui/available? false`, `:psi.ui/capabilities []`, `:psi.ui/actions []`; `:psi.ui/make-visible-action` returns an unavailable make-visible descriptor with reason `:psi.ui.unavailable.reason/no-attached-ui`; `:psi.ui/diagnostic` is nil/absent.
- Attached UI without make-visible support: `:psi.ui/available? true`; `:psi.ui.capability/make-visible` is absent from `:psi.ui/capabilities`; `:psi.ui/actions` omits any make-visible descriptor; `:psi.ui/make-visible-action` returns an unavailable make-visible descriptor with reason `:psi.ui.unavailable.reason/unsupported-capability`; `:psi.ui/diagnostic` is nil/absent.
- Provider error or invalid provider result: resolver catches the failure and returns unavailable data rather than dropping attrs from the EQL result. `:psi.ui/available? false`, `:psi.ui/capabilities []`, `:psi.ui/actions []`, `:psi.ui/make-visible-action` returns an unavailable descriptor with reason `:psi.ui.unavailable.reason/provider-error`, and `:psi.ui/diagnostic` may contain bounded redacted diagnostic text.
- Supported make-visible: `:psi.ui.capability/make-visible` is present in `:psi.ui/capabilities`; `:psi.ui/actions` contains the available descriptor; `:psi.ui/make-visible-action` returns that same descriptor; `:psi.ui/diagnostic` is nil/absent.
...
Runtime-scoped discovery uses the active UI adapter's own active/focused session identity when no `:psi.agent-session/session-id` input is supplied. The resolver is intentionally runtime-scoped and must not infer a session from arbitrary extension caller state. The provider should choose the session/buffer/window that the attached frontend currently considers active, using existing adapter state such as TUI `:focus-session-id` / context snapshot `:active-session-id` or Emacs/RPC current Psi buffer/session state. Descriptor invocation data uses only the `:psi.ui.invocation/...` namespace for correlation: if the adapter has no focused session but can still make a runtime-global UI visible, it should omit `:psi.ui.invocation/session-id` from the descriptor invocation map or set it to nil; if a runtime id is available and useful for a runtime-global action, it may include `:psi.ui.invocation/runtime-id`. Otherwise the make-visible descriptor is unavailable with `:psi.ui.unavailable.reason/no-attached-ui` or `:psi.ui.unavailable.reason/unsupported-capability` as appropriate. Available descriptors whose invocation depends on a concrete session must include correlation data under `:psi.ui.action/invocation`, preferably `:psi.ui.invocation/session-id` and, when available, `:psi.ui.invocation/runtime-id`, so a later `:psi.ui/request-action` copies the same identity into `:psi.ui.request/session-id` / `:psi.ui.request/runtime-id` on the submitted request payload. The `:psi.ui.request/...` keys are not descriptor keys; they exist only on the side-effecting request constructed from descriptor data. Emacs make-visible should target the current/active Psi buffer tracked by the Emacs/RPC frontend; the command name remains declarative, while the Emacs adapter owns resolving that active buffer at invocation time.
...
- Console follows the same capability-driven rule as TUI: if a real visibility mechanism exists, advertise it; otherwise do not pretend support. A console runtime that has a valid session/runtime `:ui-type` of `:console` but no concrete visibility mechanism is an attached UI with unsupported make-visible, not headless: the provider should report `:psi.ui/type :console`, `:psi.ui/available? true`, omit `:psi.ui.capability/make-visible`, expose no make-visible descriptor in `:psi.ui/actions`, and make `:psi.ui/make-visible-action` an unavailable descriptor with reason `:psi.ui.unavailable.reason/unsupported-capability`. If the console process has no installed active UI provider at all, it is the missing-provider/headless case instead. If a provider is installed but cannot identify any usable attached console/UI state, it is the no-attached-UI case.
