...
- [ ] Align the make-visible capability vocabulary in `design.md` so all examples and extension-usage guidance use the single resolved namespaced keyword `:psi.ui.capability/make-visible` (or explicitly mark any shorthand as non-normative), avoiding a second `:ui.capability/make-visible` contract.
