...
- [x] C3: Reconcile the Architecture-alignment claim that the post-drain
  ...
  (design.md:65, 136–137). For the N=1 `:contributions` degenerate there is no
  `:name` to key on. State in the Architecture-alignment bullet that named
  (`:prompts`) groups contribute per-prompt records keyed by `:name`, while the
  unnamed (`:contributions`) group contributes only the step-level rollup (no
  addressable per-prompt record) — so "keyed by `:name`" applies only where a name
  exists and the unified-path result shape is consistent for the degenerate case.
