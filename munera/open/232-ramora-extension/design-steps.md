...
- [ ] Remove `ramora/test` from `:test` alias `:extra-paths` in design constraint — mementum and munera are not in `:extra-paths` (only in `:deps`); design's wiring instruction diverges from the reference pattern it claims to mirror
