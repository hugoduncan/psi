...
- [ ] IR8: Make `/session-profile :clear` fail as globally reserved/unavailable even when config does not define a `:clear` profile; add command coverage proving both configured and absent `:clear` leave state unchanged and produce the reserved-profile diagnostic rather than an unknown-profile error.
