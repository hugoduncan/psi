...
- [ ] IR4: Reject canonical LLM judge `[:judge :session :session-profile]` configs and add focused validation coverage; current rejection test only places `:session-profile` beside `:type :llm`, while the closed judge `:session` schema still accepts the profile key contrary to the design's judge-profile out-of-scope boundary.
