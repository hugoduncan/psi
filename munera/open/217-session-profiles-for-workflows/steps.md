...
- [ ] DOC4: Update `doc/workflow-ir.md`'s suggested formal `session-spec` grammar so it includes canonical `:session-profile` and `:thinking-level` entries alongside `:model`, matching the implemented IR schema and the workflow authoring docs.
