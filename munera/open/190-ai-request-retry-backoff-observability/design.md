...
- a final lifecycle event records the prepared provider request as failed and final with cancellation metadata, for example `:status :failed`, `:final? true`, `:cancelled? true`, `:failure-reason :retry-cancelled`, and the last retryable error classification/cause when available;
