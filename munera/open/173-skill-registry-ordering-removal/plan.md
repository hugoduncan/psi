...
3. For branch B, implement the smallest coherent change at the registry boundary/read surface: `all-skills`, `skill-names`, and `register-skill` result `:skills` should be canonical by exact `:name` string ordering, and prompt/display/introspection projections that consume raw session `:skills` should canonicalize before exposing ordered output.
