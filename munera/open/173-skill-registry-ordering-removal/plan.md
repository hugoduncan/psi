...
3. For branch B, implement the smallest coherent change at the registry boundary/read surface: `all-skills`, `skill-names`, and `register-skill` result `:skills` should be canonical by exact `:name` string ordering, and prompt/display/introspection projections that consume raw session `:skills` should canonicalize before exposing ordered output. Prompt-component / workflow `:skill-names` filtering is an allowlist; if it renders a selected skill subset to the model, the subset should appear in canonical skill-name order rather than caller-declared or inherited parent/session order.
4. For branch C, remove insertion-order assertions from registry-level tests without replacing them with registry-level sorted-order assertions; prove unordered membership/count/exact lookup at the registry layer and add or update higher-surface tests proving canonical presentation sorting wherever ordered output is user- or model-visible, including prompt-component / workflow `:skill-names` filtered skill subsets.
