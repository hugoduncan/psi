...
- [ ] Reconcile outcome C across the task artifacts: either remove it from `design.md` as a viable branch, or add matching `plan.md` / `steps.md` / task `164` update guidance for registry-layer order-insensitive behavior with deterministic higher-layer presentation sorting.
