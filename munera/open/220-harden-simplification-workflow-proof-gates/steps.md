...
- [ ] DOC1: Update `doc/workflows.md` incidental-complexity section to document that target-present generated tasks record selector justification in `design.md`/`coverage-map.md`, including top-5 guard evidence, rejected essential false positives when present, and marginal-target concerns/falsification/scope-review questions.
- [ ] DOC2: Correct the incidental A3 gate example in `doc/workflows.md` so it uses a worktree-root-relative task artifact path (for example `munera/open/NNN-slug/before-diagnose.edn`) or is explicitly schematic; avoid implying bare `before-diagnose.edn` resolves from the worktree root.
