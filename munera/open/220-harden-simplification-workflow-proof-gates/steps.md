...
- [ ] CS2: Tighten marker-line classification for route markers so a line starting with the marker label but using whitespace before the colon (for example `PROOF_SYNC_ROUTE : COVERAGE_REVIEW` or `VALIDATION_CAPTURE_ROUTE : TERMINAL_STOP`) is reported as `:malformed-route-marker` rather than being treated as ordinary prose / missing marker; keep ordinary prose mentions without marker syntax classified as non-candidates, and add operation tests for both marker types.
