...
- [ ] CS1: Reshape deterministic workflow routing parser ownership/candidate detection: move the remaining pure routing parsers for pass-status and Munera open task path out of `workflow/core.clj` into `psi.agent-session.workflow.routing` (leaving core as registration/wiring only), and replace marker-line discovery based on `str/includes?` with an explicit line classifier that distinguishes exact markers, malformed marker attempts, and ordinary surrounding prose so allowed final-reply prose that merely mentions `PROOF_SYNC_ROUTE` or `VALIDATION_CAPTURE_ROUTE` cannot be misclassified as an ambiguous/malformed route marker.
