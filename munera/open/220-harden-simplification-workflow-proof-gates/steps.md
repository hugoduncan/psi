...
- [ ] TT3: Strengthen deterministic `workflow/munera-open-task-path-routing` operation tests beyond the current valid-path plus one extra-line invalid smoke case. Add focused tests proving malformed/non-open/absolute/raw-handoff/multiple-path or extra-prose task-path outputs route to `REPEAT`, matching the task acceptance contract that deterministic task identity routing has valid plus extra-prose/malformed regression coverage for both simplification workflows.
