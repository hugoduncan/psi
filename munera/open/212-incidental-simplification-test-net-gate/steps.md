...
- [x] TT6: Lock the transitive post-implementation test-shape gate: add workflow-definition coverage proving `review-task-implementation` includes a `review-test-shape` delegate to `review-step` with `:skill "test-shaper"`, runs after `review-task-tests` output, and resolves against real `review-step`, so the final reduce-incidental-complexity implementation-review phase cannot silently lose or retarget the test-shaper pass while focused task-212 tests remain green.
