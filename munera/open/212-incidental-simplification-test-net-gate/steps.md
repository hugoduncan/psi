...
- [ ] IR5: Reconcile the `CHANGELOG.md` Unreleased `Added` entry for `reduce-incidental-complexity`: it still says target-present runs "drive through the full `task-lifecycle`", contradicting the implemented explicit-phase topology and the `Changed` entry; update the user-facing changelog wording (or otherwise remove the contradiction) so docs/changelog match the workflow.
