...
- [ ] Make workflow-owned `:on-agent-done` stale pure results cancellation-safe: if D31 cancellation lands after `prompt-finish`'s guarded dispatch-effect stop check starts the nested `:on-agent-done` dispatch but before that handler's apply/effects, no ordinary terminal root update, background-job terminal message emission, scheduler drain, or terminal notification work may occur after the checkpoint. Derive/carry `:workflow-run-id` into `:on-agent-done`, guard its root-state update/effect vector (or route it through cancellation-entry ordering), and add a regression for cancellation between `:on-agent-done` handler-result construction and apply/effects.
