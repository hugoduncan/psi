...
- [ ] Close the call-begin/final-read→ordinary-call cancellation race: actor/judge turn starts and deterministic-operation starts currently CAS-mark `:turn-call-state` / `:operation-call-state` `:started`, perform a final stop read, then call the prompt adapter / operation handler outside the cancellation-safe boundary. Ensure a D31 cancel CAS landing after that final read but before the ordinary call cannot initiate new work (for example by making call-begin a durable in-flight marker that cancellation blocks/aborts before the call can proceed, or by moving the ordinary call under a boundary that consumes a durable cancel marker after concurrent cancel effects). Add actor, judge, and invoke regressions for cancellation after successful call-begin/final stop read but before the adapter/handler call.
