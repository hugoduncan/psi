...
- [ ] Make prompt continuation and finish stale pure results cancellation-safe: if D31 cancellation lands after `:session/prompt-continue` or `:session/prompt-finish` builds its pure result but before apply/effects, no ordinary `:runtime/prompt-continue-chain` tool-run dispatch, follow-up prompt dispatch, terminal notification/reset, or follow-up state consumption may occur after the checkpoint. Carry `:workflow-run-id` guards into those effects/root-state updates or route them through cancellation-entry ordering; add regressions for stale prompt-continue and prompt-finish pure results after cancellation.
