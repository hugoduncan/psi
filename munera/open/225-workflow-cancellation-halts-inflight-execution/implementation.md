...
## Implementation review (ψ pass 14, 2026-06-11)

Reviewed the pass-13 call-begin/call-commit changes against `task-implementation-review`, D6/D30/D31, `turn_execution_contract.clj`, `turn.clj`, `deterministic_operation_runtime/core.clj`, and the call-start regressions. Focused cancellation suites are green, but one start-boundary race remains.
...
## Implementation follow-up pass 16 (ψ, 2026-06-11)
...
## Implementation review follow-up pass 18 (ψ, 2026-06-11)
...
## Test review follow-up pass 5 (ψ, 2026-06-12)

Completed the newly added test-review follow-up for remaining task-225 cancellation regressions outside `*cancellation*test.clj`. `workflow_judge_test.clj` no longer uses `with-redefs` in the two judge retry cancellation regressions; both now use the existing nullable ctx seams `:workflow-judge-messages-fn` and `:workflow-execute-judge-turn-fn`. `workflow_runtime/statechart_runtime/step_execution_test.clj` no longer redefines `turn-execution/execute-actor-turn!` in the ranked fallback cancellation regression; it now injects `:workflow-execute-actor-turn-fn` on ctx.

Verification: `bb clojure:test:scry --namespace psi.agent-session.workflow-judge-test --namespace psi.workflow-runtime.statechart-runtime.step-execution-test` passed (27 tests, 151 assertions). `clj-kondo --lint components/agent-session/test/psi/agent_session/workflow_judge_test.clj components/workflow-runtime/test/psi/workflow_runtime/statechart_runtime/step_execution_test.clj` passed with no warnings.

## Test review (ψ pass 6, 2026-06-12)

Reviewed the task-225 test net after pass-5 nullable-seam cleanup against `task-test-review` and the design's public-routing contract (D1/D26/D37). Behaviour/race coverage is now broad and the task-specific cancellation regressions I checked use ctx/adapter nullable seams rather than global mocks.

New actionable issue: the public adapter tests for cancellation/removal assert only returned state/result shapes; they do not prove that Pathom `psi.workflow/cancel-run`, `psi-tool workflow op=cancel-run`, and `delegate remove` actually enter the canonical state-kernel events (`:psi.workflow/cancel-run` / `:psi.workflow/remove-run`) and therefore the dispatch/effects path. The dedicated dispatch tests cover the canonical events themselves, but a regression in an adapter could reintroduce direct `workflow-runtime/cancel-run` / `remove-run` mutation while still producing the same visible status. Add adapter-boundary assertions (for example event-log or dispatch seam assertions) for those public surfaces, including the `psi-tool` cancel path, so Slice 1's "public surfaces route through dispatch handlers and do not mutate inline" requirement is executable.

No tests run (review-only pass; static review of adapter tests and cancellation dispatch coverage).

## Test review follow-up pass 6 (ψ, 2026-06-12)

Closed the adapter-boundary coverage gap from pass 6. Pathom `psi.workflow/cancel-run` now asserts a `:psi.workflow/cancel-run` event-log entry with the adapter-provided `run-id`/`reason`/`session-id` plus declared cancellation effects. The psi-tool `workflow op=cancel-run` test now makes the same event-log/effect assertion while preserving the returned cancellation summary checks. The `delegate remove` adapter test now records every mutation call, asserts the only command-layer mutation call is the canonical `psi.workflow/remove-run` mutation carrying `run-id` and `session-id`, and routes that mutation through a real dispatch-backed Pathom remove adapter so the test also observes the canonical `:psi.workflow/remove-run` event-log entry plus cleanup effects. It no longer merely infers this from a boolean side effect and still proves no command-layer background-job cleanup path is invoked.

Verification: `bb clojure:test:scry --namespace psi.agent-session.mutations.canonical-workflows-test --namespace psi.agent-session.workflow-tools-test --namespace psi.agent-session.workflow-delegate-list-test` passed (37 tests / 338 assertions). Focused `clj-kondo` over the three changed test files passed with no warnings.

## Test review (ψ pass 7, 2026-06-12)
