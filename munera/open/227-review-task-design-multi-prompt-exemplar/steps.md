...
- [ ] T5: Strengthen `workflow/pass-feedback-routing` boundary tests to cover non-string prompt-reply arg values (for example numbers or vectors) and require deterministic `:invalid-pass-feedback` diagnostics rather than uncaught exceptions; adjust the routing implementation if the new test exposes a throw.
