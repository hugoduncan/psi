...
- The command is available via `M-x`.
...
1. A named interactive Emacs command exists and can be called with `M-x`.
...
Prefer a small command that delegates to the existing prompt/input boundary helpers if they already exist. If the current Emacs code already has an internal helper for keeping point in or near the input area, expose the minimal interactive wrapper rather than duplicating boundary logic.
