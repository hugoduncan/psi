...
- [ ] Migrate resolvers, prompt refresh/build paths, command surfaces, TUI projection/autocomplete, prompt request lookup, workflow child-session selection, and remaining startup/session-summary surfaces away from raw `:skills` reads and onto `:skill-ids` + registry lookup.
  ...
  - [ ] Repair remaining exact-lookup/projection seams that still fail focused verification after the membership gate restore, especially request-preparation `/skill:` expansion, discovery/detail EQL surfaces, command/help skill listings, and workflow execution adapter lookup call sites that currently pass projected skill vectors where canonical session membership context is required.
