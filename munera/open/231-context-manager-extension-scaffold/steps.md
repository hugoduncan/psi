...
- [ ] Verify that `initialized?` atom is reset correctly in all test scenarios to prevent cross-test pollution (currently handled by `use-fixtures`, but verify if any tests bypass `setup-api` or use different initialization paths)
