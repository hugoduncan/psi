...
- [ ] Make `init` idempotent for reload safety — design constraint requires clean reload without state corruption; currently `init` appends a new handler on every call (documented by `init-reload-safety-test` but not fixed); add a `compare-and-set!` guard on a private atom so repeated calls are no-ops
- [ ] Complete edge case test for partial missing keys — `handler-handles-missing-payload-keys-test` only exercises `{}` (both keys missing); add cases for `{:session-id "s1"}` (turn-id missing) and `{:turn-id "t1"}` (session-id missing) to confirm the `or` fallback works in each direction
