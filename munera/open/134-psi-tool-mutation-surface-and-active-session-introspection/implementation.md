...
- Found one actionable design/plan inconsistency: `implementation.md` records the canonical mutation seam as already discovered (`psi.agent-session.extensions.runtime-eql/run-extension-mutation-in!`), but `plan.md` still treats that seam choice as an open blocking question. Added follow-up in `design-steps.md` to align the task artifacts and make the remaining ambiguity explicit: how the chosen seam will preserve caller-supplied target `:session-id` for cross-session lifecycle mutations like `psi.extension/close-session`.
