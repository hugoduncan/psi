...
| **skill-registry** | Skills are an ordered collection | prompt-assets and discovery callers consume ordered vectors/names; tests preserve order | **Yes** | prompt rendering and discovery are order-sensitive | shared ordered collection helpers |
...
- `command-registry`
  ...
  - first-seen name wins in merged reads
- `skill-registry`
  - first-registration order preserved
- `prompt-registry`
...
- `skill-registry`
  - session-local ordered collection of registered skills for prompt/discovery use
- `prompt-registry`
