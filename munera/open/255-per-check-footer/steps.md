...
- [ ] Restore the existing blank separator after a failed command with a non-empty per-command `:footer`; rendering currently leaves only one newline before the next failure section or global trailer, contrary to the documented prompt shape. Update the exact boundary tests to protect the separator.
