...
- [ ] Add a handler-level regression test for a non-empty per-command `:footer` with `:max-output-chars` truncation, proving the footer follows the truncated-output marker and still precedes the global trailer.
