...
- [ ] Add handler-level coverage for a multi-line non-empty per-command `:footer`, proving its complete text is rendered verbatim after that command's output and before the next section or global trailer.
