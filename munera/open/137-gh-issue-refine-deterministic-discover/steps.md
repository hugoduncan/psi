...
- [ ] **II** Integration test CC: add `:run-id "run-github-find-issue"` assertion to the invocation-shape check in `find_issue_integration_test.clj`. The stub should capture `(:workflow-run-id invocation)` as `:run-id` (matching the reference pattern in `workflow_invoke_runtime_test.clj`) and assert `(= "run-github-find-issue" (:run-id invocation))`.
