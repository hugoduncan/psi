...
These reviews should mirror the repeat-until-no-actionable-feedback style used by `review-task-implementation`. This task should introduce a separate implementation architecture review skill (candidate name: `review-implementation-architecture`) and route it as an explicit post-implementation gate before or alongside test-shaper and code-shaper. `implement-task` output is not considered complete until implementation architecture, test-shaper, and code-shaper review feedback has been resolved.
