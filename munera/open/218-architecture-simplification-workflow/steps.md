...
- [ ] **IR1 — Harden live architecture-targets test for absent bb/Gordian.** `architecture-targets-live-envelope-shape-test` currently calls `shell/sh "bb" ...` directly and only skips on non-zero exit; if `bb` is absent or shell execution throws, the test errors instead of honoring the plan/design requirement to skip or fixture-fallback when `bb`/Gordian are unavailable. Wrap the command in a safe availability helper or `try`/`catch` path and keep unavailable/non-zero behavior non-failing.
- [ ] **IR2 — Lock interpretable candidate id/type shape in the live envelope test.** The live `architecture-targets --edn` shape test currently checks only that `:candidate/id` and `:candidate/type` are present/non-nil. Strengthen it to assert an interpretable stable shape when `:winner` is present, for example `:candidate/type` is a keyword in the supported architecture candidate set and `:candidate/id` is a vector whose tag/type is consistent enough for selector membership resolution, without asserting repository-specific values, scores, labels, counts, or ordering.
