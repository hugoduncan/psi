...
- [x] **IR1 — Harden live architecture-targets test for absent bb/Gordian.** `architecture-targets-live-envelope-shape-test` currently calls `shell/sh "bb" ...` directly and only skips on non-zero exit; if `bb` is absent or shell execution throws, the test errors instead of honoring the plan/design requirement to skip or fixture-fallback when `bb`/Gordian are unavailable. Wrap the command in a safe availability helper or `try`/`catch` path and keep unavailable/non-zero behavior non-failing. Done: added `architecture-targets-command` safe helper plus unavailable-command coverage; live check now treats thrown `IOException`/non-zero as non-failing skip.
- [x] **IR2 — Lock interpretable candidate id/type shape in the live envelope test.** The live `architecture-targets --edn` shape test currently checks only that `:candidate/id` and `:candidate/type` are present/non-nil. Strengthen it to assert an interpretable stable shape when `:winner` is present, for example `:candidate/type` is a keyword in the supported architecture candidate set and `:candidate/id` is a vector whose tag/type is consistent enough for selector membership resolution, without asserting repository-specific values, scores, labels, counts, or ordering. Done: live winner now must satisfy supported architecture candidate type and selector-interpretable id shape without asserting repository-specific ranking values.

## Runtime smoke follow-ups

- [x] **IR1 — Make `extract-task-path` routing deterministic.** Runtime smoke showed valid extracted path `munera/open/219-simplify-rpc-session-family` was misrouted to `terminal-stop-summary` because the path-shape check used an LLM judge. Replace it with deterministic routing and lock valid-path/extra-prose cases in tests.
