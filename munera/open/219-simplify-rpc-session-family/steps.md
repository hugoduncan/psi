...
- [ ] TT4: Update `coverage-map.md` after TT3 so the authoritative coverage artifact records callback-backed command-op `/new` coverage in `psi.rpc-session-navigation-test/rpc-session-resume-and-rehydrate-events-test` and the latest focused suite count (59 tests / 469 assertions), rather than the stale 59 / 459 assertion count and prompt-only callback gap disposition.
