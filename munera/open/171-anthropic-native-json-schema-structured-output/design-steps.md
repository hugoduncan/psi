...
- [ ] Align Anthropic JSON Schema `:output_format :strict` wording: choose whether requests always send `:strict true` or reflect normalized `[:structured-output :strict?]`, then update design/plan/test expectations so the request contract has one interpretation.
