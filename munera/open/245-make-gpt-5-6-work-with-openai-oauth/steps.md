...
- [ ] Route turn-runtime unsupported runtime model preflight errors through the shared `psi.ai.model-registry/unsupported-runtime-model-message` helper and update regression coverage, so a persisted or startup-selected OAuth-backed `gpt-5.6` produces the same clear unsupported-model message as `/model`, RPC `set_model`, RPC picker selection, and TUI picker selection instead of a drift-prone raw `:runtime/unsupported-message`.
