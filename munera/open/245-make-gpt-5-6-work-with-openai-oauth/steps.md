...
- [ ] Reject or skip `:runtime/unsupported?` models in the cycle-model path (`components/agent-session/src/psi/agent_session/session_settings.clj` and RPC `cycle_model`) with regression coverage, rather than allowing an OAuth-backed scoped `gpt-5.6` to be selected by cycling and only fail later at turn preflight.
