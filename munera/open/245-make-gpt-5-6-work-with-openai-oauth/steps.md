...
- [ ] Consolidate the duplicated unsupported-runtime-model message formatting used by agent-session `/model`, RPC picker selection, and TUI picker selection behind one shared helper so future unsupported runtime policies cannot drift across command surfaces.
