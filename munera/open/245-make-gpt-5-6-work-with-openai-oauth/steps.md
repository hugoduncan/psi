...
- [ ] Reject `:runtime/unsupported?` models in the RPC `set_model` path (`components/rpc/src/psi/rpc/session/ops.clj`) with a clear request error and regression coverage, rather than persisting the catalog-facing unsupported runtime model as if selection succeeded.
