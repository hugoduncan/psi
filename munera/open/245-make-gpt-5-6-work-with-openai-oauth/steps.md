...
- [ ] Reject unsupported runtime models in TUI picker-backed model selection (`components/app-runtime/src/psi/app_runtime/tui_frontend_actions.clj`) with a clear unsupported-model result and regression coverage, rather than persisting an unsupported OAuth-backed `gpt-5.6` selected from the model picker.
