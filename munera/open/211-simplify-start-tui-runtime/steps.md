...
- [ ] TT3 Add a public-path characterization test for `start-tui-runtime!` TUI navigation/action callback wiring: capture the TUI opts, lock `:frontend-action-handler-fn!`, `:resume-fn!`, `:switch-session-fn!`, and `:fork-session-fn!` as callable public opts, and exercise at least one navigation callback or frontend session action through those opts to prove focus updates and a `:context-updated` event is emitted without stubbing the navigation helpers.
