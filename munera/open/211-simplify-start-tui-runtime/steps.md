...
- [ ] TT16 Add a public-path characterization test for `start-tui-runtime!` TUI terminal mode option forwarding: capture public TUI opts and assert `:alt-screen false` is present, so dropping the option and falling back to `psi.tui.app/start!`'s default true alt-screen behaviour fails through public startup.
