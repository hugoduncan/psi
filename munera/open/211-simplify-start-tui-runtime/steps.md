...
- [ ] TT10 Add a public-path characterization test for `start-tui-runtime!` frontend action and remaining navigation callback semantics: drive captured `:frontend-action-handler-fn!` with a real `:select-session` action result and/or direct `:fork-session-fn!`/`:resume-fn!` inputs against real session state, then assert focus, returned rehydrate state, emitted `:context-updated` events, and session/model state so a callable-but-miswired handler, fork, or resume callback fails rather than only `:switch-session-fn!` being covered.
- [ ] TT11 Add a public-path characterization test for the non-streaming branch of `start-tui-runtime!` `:on-queue-input-fn!`: exercise the captured callback while the focused session is idle/non-streaming and assert the queued follow-up message text plus `"Queued follow-up message."`, so a regression that always uses the streaming steering branch remains covered through public TUI startup opts.
