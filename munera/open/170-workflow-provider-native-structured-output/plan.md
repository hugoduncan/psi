...
2. Add a small workflow-runtime helper that turns a structured output spec into the task-169 AI request map under `:structured-output`, failing clearly when `:json-schema` is missing or required native support cannot be requested.
...
- `:require-provider-native? true` forbids fallback even if `:fallback :prompted-json` is present.
