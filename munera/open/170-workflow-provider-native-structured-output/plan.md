...
3. Extend the bounded turn-execution contract so `execute-actor-turn!` / `execute-judge-turn!` accept an optional fourth `opts` argument, forward it unchanged to `execute-session-turn!`, canonical execution results may carry `:execution-result/structured-output`, and bounded actor/judge results expose that value as top-level `:structured-output`.
...
5. Wire session-step execution so structured `:outputs` pass that request map as `{:structured-output ...}` opts into actor-turn generation, preserving the same opts across ranked model fallback candidates, and read strategy/payload metadata only from the top-level bounded turn result `:structured-output` seam.
6. Wire LLM-judge execution so judge structured `:outputs` pass the same request map as `{:structured-output ...}` opts into the initial judge-turn generation, keep structured failures out of prose no-match retry loops, and read the same result seam.
...
- The authoritative workflow-to-turn request seam is an optional fourth `opts` argument on `execute-actor-turn!` / `execute-judge-turn!`; task 170 passes only the provider-neutral `{:structured-output ...}` map there, and the aliases forward it unchanged to `execute-session-turn!`. Ranked model fallback reuses the same opts for each candidate; structured judges do not use prose no-match retries for structured-output contract failures.
