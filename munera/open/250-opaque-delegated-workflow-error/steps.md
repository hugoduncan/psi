...
- [ ] Add pure tests for placeholder-only fallback, redactable-but-actionable messages, actionable terminal fallthrough, target/step escaping, exact 512-code-point boundaries, Unicode code points, and idempotent nested-message sanitization.
  - Proof covers terminal fallthrough, redactable-but-actionable messages, non-actionable-target fallback, supplementary-Unicode code-point truncation, malformed-nested ordinary-message sanitization, escaped multiline quoted credentials, U+0085 whitespace/path-boundary handling, and empty quoted credential values (which remain unredacted because they are ineligible). Expand the remaining boundary matrix.
