...
- [x] Replace the separator scan/query counter assertions with proof that accounts for the work performed inside each separator query. `sanitize-component-analysis` counts only the eager indexing pass and the number of query invocations, so changing each query back to a linear suffix search while retaining those counter increments would leave the regression green even though total work is quadratic. Use a deterministic work measure that includes examined input positions, or isolate a calibrated scaling check in an explicit slow/benchmark layer; keep the public output assertions independent of the complexity mechanism.
  - `sanitize-component-analysis` now reports raw input positions examined by separator indexing and lookup together. Constant-time cached-index queries examine no additional positions, and symmetric late-slash/backslash cases prove exactly one examined position per input character while separate public assertions retain the bounded output contract.

## Latest test-shaper review follow-ups

- [x] Replace the self-reported separator-work assertion with regression proof that fails if cached separator queries become repeated suffix searches. `path-separator-examined-position-count` is incremented only during the unconditional eager indexing pass, so restoring `.indexOf` or another linear search inside each query while retaining that pass leaves the asserted count equal to the input length and keeps both late-separator output cases green. Route query character examination through the measured work seam so the count necessarily includes lookup work, or move a calibrated scaling check to an explicit slow/benchmark layer; avoid exposing production analysis API solely to assert an implementation-maintained counter.
  - A counting `CharSequence` now observes actual separator-scanner character reads: indexing examines each position once and thousands of slash/backslash queries examine none. The production-only analysis API and self-maintained counter were removed.

## Final test-shaper re-review follow-ups

- [ ] Replace the direct `#'delegated-failure/path-separator-scanner` test and `CharSequence.charAt` interaction count with proof at a behavior-bearing boundary. The current assertion exercises a private helper independently of `sanitize-component`, required changing that helper's production signature for the test double, and remains green if the sanitizer stops using the helper or acquires quadratic work elsewhere. Keep the slash/backslash public output cases, and put calibrated scaling protection in an explicit slow/benchmark layer unless a deterministic public work invariant can cover the complete sanitizer path without asserting implementation interactions.
