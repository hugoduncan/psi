...
- [ ] **TS-S1:** Extract the A2 relocation-guard content-locks (`task_209_workflow_definitions_test.clj`
      ...
      corrected per-unit A2 relocation guard"` block, leaving the task-209 selection/baseline
      asserts in place. Single-concern + correct concern naming (the current label
      "preserves … baseline contracts" misdescribes the new-behaviour A2 locks), consistent
      with the file's per-concern `testing` grouping (clean-baseline/coverage-review/diff-gate).
