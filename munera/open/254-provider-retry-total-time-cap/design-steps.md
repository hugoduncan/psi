...
- [ ] **Pin what `:max-retries` reports in the retry-outcome and `provider_request_finished` event under the sentinel-`nil` default.**
...
- [ ] **Reconcile the cancel path's deadline clearing with the per-sleep clear that it reuses (and the test-mode skip).**
