...
2. Gather evidence for the ChatGPT/Codex endpoint contract.
   ...
   - If feasible, run guarded live or fixture-backed probes against `https://chatgpt.com/backend-api/codex/responses` to determine whether native schema-constrained structured output is actually supported.
   - Capture the exact request and response shape evidence that justifies the chosen capability outcome.

3. Implement the smallest coherent capability slice for the evidence-backed outcome.
   - If native support is verified, introduce a distinct Codex native mechanism unless the protocol is proven identical to an existing OpenAI mechanism, then update capability declaration, strategy selection, request shaping, response extraction, and non-streaming support only as warranted.
   - If native support is not verified, keep Codex explicitly fallback-only and strengthen code/tests/docs so that outcome is evidenced and inspectable rather than assumed.
...
2. Capability discovery slice: determine the real ChatGPT/Codex native structured-output contract, if any.
3. Capability implementation slice: implement the minimal coherent native or explicit-fallback outcome.
