...
2026-05-21 root-registry migration pass:
- Refactored `components/workflow-registry/src/psi/workflow_registry/registry.clj` into a workflow-specific adapter over `psi.root-registry.registry` with declared registry id `:workflow-definitions` and adapter-owned entry projection `{:id definition-id :extension-id :workflow-definition :value stored-definition}`.
- Preserved workflow-registry boundary semantics at the adapter: id normalization, blank-id UUID generation, target-authored validation, replace-on-register, nil lookup miss, sorted `list-definitions`/`definition-ids`, throw-on-miss removal, and the tuple-shaped pure return contracts for register/remove.
- Preserved the canonical persisted compatibility path `[:workflows :definitions]` by re-projecting root-registry contents back onto that path after register/remove, so direct-path compatibility consumers in `session-state`, workflow model docs, and integration tests remain coherent while root-registry becomes the lower storage owner.
- Added focused migration-guard coverage in `components/workflow-registry/test/psi/workflow_registry/registry_test.clj` proving root-registry declaration, hidden raw entry shape behind the public workflow API, and canonical-path coherence after register/replace/remove.
...
- Remaining follow-up: explicitly classify whether any non-compat higher semantic read/projection seam still reads `[:workflows :definitions]` directly and should be redirected, or confirm none remain and close the final unchecked step.
