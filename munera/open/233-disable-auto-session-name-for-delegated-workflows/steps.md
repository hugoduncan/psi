...
- [ ] Add regression coverage proving each workflow ownership id (`:workflow-run-id`, `:workflow-step-id`, and `:workflow-attempt-id`) independently makes an otherwise resolved root session ineligible; current tests only cover all workflow ids together with `:workflow-owned? true`.
