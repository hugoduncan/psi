...
- [ ] Add regression coverage proving `:workflow-owned? true` independently makes an otherwise resolved root session ineligible; current coverage combines the flag with workflow ids, so it does not isolate the explicit boolean ownership marker required by the design.
