...
- [x] Add resolver/API projection coverage for `agent-session-workflow-linkage` proving extension-safe session queries expose `:psi.agent-session/parent-session-id` together with the workflow ownership attrs for both ordinary root sessions and child/workflow-owned sessions; the implementation changed the resolver output, but current tests only exercise nullable extension fixtures rather than the real query surface required by the design.
- [x] Remove or consolidate duplicate `agent-session-workflow-linkage-query-test` coverage between `components/agent-session/test/psi/agent_session/eql_introspection_test.clj` and `components/agent-session/test/psi/agent_session/workflow_linkage_query_test.clj`; keep one authoritative test home so the projection contract is not maintained in two places.
