...
Applied `code-shaper`: re-read task artifacts, `doc/scheduler.md`, scheduler `resolve-fire-time!`/`millis-until`/`validate-delay-ms!`, session-config parsing, and `psi-tool-scheduler-at-resolution-matrix-test`. The scheduler code remains simple and locally comprehensible for this doc-only slice: absolute `:at` resolution is isolated in `resolve-fire-time!`, delay truncation is explicit in `millis-until`, bounds validation is single-sourced through `validate-delay-ms!`, and supported `session-config` keys are defined once and match the user-facing doc. Focused verification stayed green: `clojure -M:test --focus psi.agent-session.psi-tool-scheduler-test/psi-tool-scheduler-at-resolution-matrix-test` → 1 test, 21 assertions, 0 failures; targeted `clj-kondo` over scheduler source/test → 0 errors, 0 warnings. No new actionable code-shaping issues found.
