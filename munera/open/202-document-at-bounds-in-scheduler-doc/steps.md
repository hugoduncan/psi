...
- [x] Fix `doc/scheduler.md` supported `session-config` key list: it documents `:tool-defs`, but `parse-session-config!` accepts `:tool-ids` and rejects unsupported keys.
