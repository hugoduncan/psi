...
- [ ] B1 — Resolve the dual-kind command case. `/project-repl` is in BOTH
...
- [ ] B2 — Pin the canonical spec-table **key form** (leading-slash vs bare).
...
- [ ] B3 — Specify what `format-help` derivation must and must not cover: whether
...
- [ ] B4 — State whether the single keyed spec table is an **ordered** map and
...
- [ ] B5 — Decide at design (not deferred to plan) whether the spec carries a
