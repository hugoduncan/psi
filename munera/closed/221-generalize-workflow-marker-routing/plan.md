...
- Removal sequencing is compile-safe and repository-coherent: Slice 2 may make the generic parser public and parameterized, but the old proof-sync / validation-capture wrappers and route constants remain as temporary delegators until the handoff commit. The handoff must add `workflow/exact-marker-routing`, migrate both checked-in simplification workflow EDNs to that operation, and remove the two old registrations, wrappers, and constants in the same commit. Do not commit a state where `workflow.core` references deleted routing vars or where checked-in workflow EDNs still call removed operation ids.
...
4. **Coherent operation/EDN handoff** — in one repository-state handoff commit, register `workflow/exact-marker-routing`, update the handler to pass resolved args directly to `routing/parse-exact-marker-routing`, migrate `reduce-architectural-complexity.edn` and `reduce-incidental-complexity.edn` disposition steps to invoke the generic operation with workflow-owned `:marker-label` and `:allowed-routes` args while preserving `:on` topology, then remove `workflow/proof-sync-disposition-routing`, `workflow/validation-capture-disposition-routing`, `proof-sync-routes`, `validation-capture-routes`, `parse-proof-sync-disposition-routing`, and `parse-validation-capture-disposition-routing`. Update built-in operation smoke tests for registration, valid invocation, invalid-arg invocation, and old-id absence. This slice must not be split into a committed old-id-removal state before the EDN migration.
5. **Post-handoff EDN verification** — parse-read both simplification workflow EDNs, verify all disposition `:on` maps are unchanged after operation migration, and grep to confirm no checked-in simplification workflow still references the removed operation ids.
