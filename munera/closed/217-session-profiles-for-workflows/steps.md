...
- [x] DOC2: Update `doc/workflow-ir.md` delegate semantics to remove the stale statement that delegate steps do not use delegated session overrides; document the current canonical `[:delegate :session]` config for `:session-profile`, `:model`, and `:thinking-level`, and that it shapes only the delegated run's concrete inherited defaults rather than constructing a delegate actor session.
  - Done: `doc/workflow-ir.md` now shows optional delegate `:session` config, documents the allowed `:session-profile`/`:model`/`:thinking-level` keys, clarifies that the config shapes only delegated-run `:inherited-defaults` rather than a delegate actor session, and updates the suggested IR grammar.
- [x] DOC3: Update `doc/workflow-grammar.md`'s formal `delegate-step` production so it explicitly permits the compact delegate session config keys `:session-profile`, `:model`, and `:thinking-level`; keep direct `:speed-mode`/`:effort-override` excluded and align the formal grammar with `doc/workflow-ir.md`'s delegate `[:delegate :session]` semantics.
  - Done: `doc/workflow-grammar.md` now includes `delegate-session-config-entry*` in the formal `delegate-step` production and defines the allowed compact delegate keys as exactly `:session-profile`, `:model`, and `:thinking-level`, with direct authored `:speed-mode`/`:effort-override` still excluded and semantics aligned to delegate inherited-default shaping.
...
- [x] CS2: Move the shared selectable session-profile name grammar out of `psi.workflow-registry` into a neutral low-level session-profile/session-state namespace or component, then update shared-config, workflow-loader, and workflow-runtime callers to consume that neutral owner and remove the `shared-config -> workflow-registry` dependency introduced only for name validation.
  - Done: moved the selectable profile-name grammar into new neutral component `psi/session-profile` namespace `psi.session-profile.names`; updated shared-config, workflow-loader markdown parsing, workflow-runtime IR schemas, and target IR compiler to consume it; removed the incidental `shared-config -> workflow-registry` dependency in favor of `shared-config -> session-profile`; added the component to root deps/source paths and test source paths.
- [x] CS3: Centralize the complete selectable session-profile name predicate (token grammar plus reserved-name exclusion) in the neutral `psi.session-profile.names` owner, use it from shared-config and workflow IR/compiler/frontmatter boundaries, and add coverage proving workflow session/delegate/markdown `:session-profile :clear` is rejected before run creation rather than compiling into a run that fails only during step resolution.
  - Done: `psi.session-profile.names` now owns `reserved-profile-names`, `reserved-profile-name?`, and complete `valid-profile-name?` semantics; shared-config aliases the neutral reserved set, workflow IR/compiler boundaries reject reserved `:clear`, and workflow-loader grammar coverage proves session/delegate/markdown `:session-profile :clear` fails before run creation.
- [x] CS4: Move the pure resolved-profile selection helper used for already-captured session-profile snapshots (currently `find-valid-profile` in `psi.shared-config.session-profiles`) into the neutral `psi.session-profile` owner, update shared-config/commands/workflow-step-session-config callers or aliases, and remove the `workflow-step-session-config -> shared-config` dependency so deterministic workflow step resolution does not depend on config IO/model-registry ownership.
  - Done: added neutral `psi.session-profile.selection/find-valid-profile` for already-resolved profile maps/snapshots, kept `psi.shared-config.session-profiles/find-valid-profile` as an alias for command/config callers, updated workflow step session-config resolution to require the neutral selector directly, and removed `psi/shared-config` from `components/workflow-step-session-config/deps.edn`.
