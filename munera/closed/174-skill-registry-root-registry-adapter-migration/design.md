...
- Bootstrap or context construction paths that supply startup/default skill maps should hydrate the root skill-definition registry directly before sessions are created, so ordinary top-level session creation can usually write canonical `:skill-ids` membership without acting as the bootstrap path for definitions.
- Non-bootstrap session-creation and membership-replacement paths may still be supplied concrete skill maps (for example scheduler-created sessions, workflow child-session creation, explicit child-session mutation inputs, or direct `:session/set-skills` replacement). Those paths must not reject valid-but-not-yet-registered skills and must not require a separate preparatory dispatch/effect. Instead, they should normalize supplied skill maps synchronously inside the same root-state update that creates or rewrites session membership: ensure/insert each missing definition into root-registry, derive canonical skill ids, then persist only session `:skill-ids`.
- Name-only selection paths are narrower: when a lifecycle or workflow path is supplied only skill ids/names rather than full skill maps, it may assume those ids already exist in the authoritative parent session membership or root-registry definition store; missing-name handling should preserve today's exact-lookup semantics for that path rather than inventing placeholder persisted definitions.

This keeps lifecycle behavior deterministic, synchronous, and replayable: every lifecycle handler's root-state transform leaves canonical root-registry definitions and session membership populated, with no embedded `:skills` projection and no required follow-up hydration pass.
