...
- [x] Add an explicit implementation step to replace the current startup `agent_session/bootstrap.clj` `:session/register-skill` loop with direct root-registry skill-definition hydration before any session membership is created, so task steps match the bootstrap ownership split required by the design and plan.
