...
- [x] Fix the stale `workflow-registry` lower-result test so it asserts against the canonical `root-registry/lookup` result shape (`[:result :value]`) instead of the nonexistent nested `:entry` field, keeping the review proof aligned with the live lower contract.
