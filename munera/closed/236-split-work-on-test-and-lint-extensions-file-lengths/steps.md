...
- [ ] DT4: Update `README.md` so the `bb commit-check:file-lengths` behavior change is discoverable from the primary documentation: either summarize that it scans `components/`, `bases/`, and `extensions/` `src`/`test` paths with legacy oversized extension ratchets, or link directly to the updated detailed docs.
