...
- [x] A1/A2 unit identity includes `line` (target pinned to `(..., 4, 12)`), but the
  ...
  - Resolved: A1/A2 match units by line-insensitive key `(ns, var, arity)`; `line`
    kept only as selector label. Collision fallback documented.
- [x] A2 does not define `before(u)` for newly-created helper units absent from
  ...
  - Resolved: baseline-absent units take `before(u) := 0`, so new seams enter `T`
    and count in `sum_after`.
