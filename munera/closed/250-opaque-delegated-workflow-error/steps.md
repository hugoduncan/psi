...
- [x] Make `sanitized-component` evaluate span recognizers in lexical-precedence order and stop after the first match instead of eagerly computing stack-frame, credential, token, and path candidates before the precedence `cond`. The current coordinator obscures its first-match contract, performs lower-priority scans even when a higher-priority span already wins, and can advance stateful quote/path scanners for candidates whose results are discarded. Preserve the exact redaction behavior and add an overlap/precedence regression that would detect lower-priority recognizer evaluation affecting subsequent scanning.
  - `first-sanitizer-span` now short-circuits stack, credential, bearer, prefixed-token, path, and placeholder recognition in canonical order. A prefixed-token/path overlap followed by a secret-relative path in the same delimited run proves the first span wins without disturbing subsequent path redaction.
