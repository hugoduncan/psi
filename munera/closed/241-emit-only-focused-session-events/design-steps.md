...
- [x] Reconcile plan "Key decision 1" (immutable `:default-session-id` frozen at `make-rpc-state`) with the design's fallback definition `default-session-id-in` (a *live* function of the current ctx session list = first-listed session). Two unresolved points: (a) verify/justify that the construction-time `session-id` from `session-ctx-factory` equals `default-session-id-in` at setup (plan asserts this but steps.md has no verification step); (b) specify the intended behaviour when the session set changes over the connection lifetime while `focus-session-id` is `nil` — the frozen `:default-session-id` diverges from the design's live `default-session-id-in` (e.g. initial default session closed, or a new session inserted first). Decide whether the frozen value is intentionally stale-but-acceptable or must track the live first-listed session, and align plan + design accordingly.
