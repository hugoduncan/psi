...
- [x] Clarify the Anthropic JSON Schema native non-streaming boundary: either task 171 adds an Anthropic provider `:execute`/non-streaming path that returns top-level `:structured-output`, or it limits non-streaming work to request/response extraction helpers/tests without changing the provider execution surface.
- [x] Align Anthropic JSON Schema `:output_format :strict` wording: choose whether requests always send `:strict true` or reflect normalized `[:structured-output :strict?]`, then update design/plan/test expectations so the request contract has one interpretation.
