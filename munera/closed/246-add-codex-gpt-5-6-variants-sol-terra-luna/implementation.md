...
## Task-test-review (2026-07-28, 4th pass)

- no new steps: task 246 tests are complete and well-formed at HEAD. All three suites pass — `psi.ai.model-registry-test` 270/25, `psi.rpc.session.command-pickers-test` 9/2, `psi.turn-runtime.response-mode-test` 150/20. Design acceptance behaviours all covered (catalog metadata; OAuth/Codex verbatim 3-facet routing; gpt-5.5 control; bare-gpt-5.6 negative; offer/accept/preflight surfaces). Infra deps nullable (`create-null-context`), injected capturing `emit!`, no mocks/stubs; the `with-redefs [execute-live-turn!]` provider-boundary stub in the preflight positive test is the module's established seam (18 pre-existing uses), not a task-introduced regression.
- out-of-scope observation (not a task 246 artifact, no step added): the worktree has *uncommitted, unrelated* WIP in `response_mode_test.clj` + `response_mode_retry_test.clj` (an OpenAI usage-limit retry test moved between files) that currently leaves `response_mode_test.clj` with an unbalanced form (reader error at ~403:73), so a focused run of that ns fails to load in the dirty tree. Stashing the WIP confirms the committed task-246 state loads and passes cleanly. Left untouched — belongs to separate work, not this task.
...
Closed 2026-07-28: completed. All acceptance criteria met — sol/terra/luna
added to the catalog and OpenAI OAuth/Codex route (verbatim ids), backed by live
streaming-probe evidence; bare `gpt-5.6` remains unsupported (task-245 behaviour
preserved). Tests, CHANGELOG, and user docs updated; review and docs-review
gates converged with no new feedback. Knowledge extraction ran and correctly
deduplicated against the existing task-245 memories (no new artifacts).
