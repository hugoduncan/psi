...
- [x] CS2: Collapse the `/new` command-result side-effect sequence into one narrow shared RPC command helper that owns the `:new-session` type check, focus movement, and rehydration emission order. Replace the duplicated `when (= :new-session (:type cmd-result))` / `focus-new-session-command!` / `emit-new-session-command!` sequence in `commands.clj` and `prompt.clj` with that helper, and make `new-session-command-payload` private if it remains only helper-internal. Preserve existing prompt-op and command-op `/new` observable behaviour; do not move adapter-neutral semantics into RPC. Done 2026-06-07: added public `rpc.commands/handle-new-session-command-result!` to own the `:new-session` predicate, RPC focus movement, and rehydration emission order; made `new-session-command-payload`, `focus-new-session-command!`, and `emit-new-session-command!` private; updated command-op and prompt-op paths to call the one helper before their respective command-result rendering. Verification green: prompt/navigation subset 13 tests / 146 assertions, pinned focused RPC suite 59 tests / 469 assertions, targeted clj-kondo 0/0, `git diff --check` clean.
