...
- [ ] Tighten `format-compile-error-with-step-context-test`: assert the combined substring `"Step 'my-step' (index 2): Unsupported target workflow step type"` appears on a single line, not just as two independent `contains-line?` checks
- [ ] Add path-presence assertion to the `"real Malli explain-data"` testing block in `format-structural-errors-test`: assert the output contains a path segment (e.g. `"steps"` or `"contributions"`) — design criterion #4 requires "includes a path or field name"
- [ ] Fix the `"non-prior step ref"` integration test in `create-run-surfaces-step-contextual-message-test`: use a two-step workflow where step-b forward-references step-a so referrer and target have distinct names, or narrow the test description to "self-reference is detected"
- [ ] Strengthen `format-multiple-errors-test`: add per-entry assertions for constraint text (not just step names); add a mixed compile-error + semantic-errors test case
