...
- [x] **Pin the minimum per-attempt delay when a provider-sent `Retry-After` is non-positive (integer 0 or negative).**
...
## Inconsistency review 2026-08-18 (plan review, second turn)

- [x] **Plan.md test 6 promises cancellation coverage for the truncated final sleep that no test exercises.**
  Plan.md test 6 states "Cancellation interrupts a pending backoff (incl. truncated final sleep) → `:retry-cancelled`, and no stale `:retry-deadline-ms` leaks after cancel." But no test in `response_mode_retry_test.clj` or `response_mode_test.clj` drives a cancel during the truncated final sleep: `execute-prepared-request-cancel-clears-deadline-test` (retry_test.clj:319) uses the 10-minute default budget (600000) so the first 2 s delay never overshoots — it cancels during a normal backoff; `execute-prepared-request-cancels-pending-retry-backoff-test` (response_mode_test.clj:583) cancels during the first backoff under an explicit cap. The final-sleep cancel behavior IS implemented (core.clj final-sleep branch re-checks the `cancelled?` return from `sleep-for-retry!` and routes to `:retry-cancelled` + `:clear-deadline? true`), and design.md "Cancellation during the truncated final sleep" specifies it — but the plan-declared test coverage for that path is missing. Add a test with a small budget whose first delay overshoots (e.g. `:auto-retry-total-timeout-ms 5000` + base 2000) and a `:provider-retry-cancelled?`/sleep-fn that flips during the truncated sleep, asserting `:retry-cancelled`, no stale `:retry-deadline-ms`, and the final-sleep path's truncated `provider_retry_scheduled` followed by `provider_request_cancelled`.
