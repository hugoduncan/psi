...
Completed the newly added prompted-JSON fallback streaming coverage. Added Codex fallback streaming assertions for first-class `:structured-output-strategy` and parsed `:structured-output-result` from streamed JSON text, and added fallback-only Anthropic streaming coverage proving strategy emission while text remains ordinary text-delta without synthetic forced-tool result extraction. Implemented Codex streaming result accumulation/emission for prompted-JSON fallback. Verification: `clojure -M:test --focus psi.ai.providers.openai-structured-output-test --focus psi.ai.providers.anthropic-structured-output-test` => 10 tests, 44 assertions, 0 failures; `clojure -M:test --focus psi.ai.providers.openai-structured-output-test --focus psi.ai.providers.anthropic-structured-output-test --focus psi.ai.model-registry-test --focus psi.ai.user-models-test` => 34 tests, 204 assertions, 0 failures.
Lint: `clj-kondo --lint components/ai/src/psi/ai/providers/openai/codex_responses.clj components/ai/test/psi/ai/providers/openai_structured_output_test.clj components/ai/test/psi/ai/providers/anthropic_structured_output_test.clj` => 0 errors, 0 warnings.

## 2026-05-23 — code-shaper review

Found one new actionable code-shaping issue: Anthropic prompted-JSON fallback streaming diverges from the other structured-output streaming paths. It emits `:structured-output-strategy` and ordinary text deltas, but never accumulates/parses the fallback JSON text into a first-class `:structured-output-result`, while OpenAI Chat Completions and Codex prompted/native streams do. This weakens data-shape consistency for callers and leaves fallback streaming result handoff provider-dependent.

## 2026-05-23 — code-shaper follow-up

Completed the Anthropic prompted-JSON fallback streaming result follow-up. Anthropic streaming now accumulates fallback text deltas while preserving ordinary `:text-delta` events, emits parsed first-class `:structured-output-result` with `:source :prompted-json/text` before completion when JSON extraction succeeds, and guards result emission to once per stream. Updated focused fallback streaming coverage to assert strategy, ordinary text delta preservation, parsed payload, and raw payload. Verification: `clojure -M:test --focus psi.ai.providers.anthropic-structured-output-test` => 5 tests, 23 assertions, 0 failures. Lint: `clj-kondo --lint components/ai/src/psi/ai/providers/anthropic.clj components/ai/src/psi/ai/providers/anthropic/structured_output.clj components/ai/test/psi/ai/providers/anthropic_structured_output_test.clj` => 0 errors, 0 warnings. Combined focused verification: `clojure -M:test --focus psi.ai.providers.openai-structured-output-test --focus psi.ai.providers.anthropic-structured-output-test --focus psi.ai.model-registry-test --focus psi.ai.user-models-test` => 34 tests, 204 assertions, 0 failures.
