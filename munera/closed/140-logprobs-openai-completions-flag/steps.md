...
- [x] **1. Schema** — add to `session_state/model.clj`:
...
- [x] **2. Request building** — `chat_completions.clj` `build-request`:
...
- [x] **3. SSE extraction** — `chat_completions.clj`:
...
- [x] **4. Turn-runtime accumulation** — extend accumulator:
...
- [x] **5. Journal append + telemetry** — `prompt_recording.clj` `build-record-response`:
...
- [x] **6. Journal projection** — `prompt_request.clj` `journal->provider-messages`:
...
- [x] **7. EQL resolver** — add `:psi.agent-session/last-turn-logprobs` resolver

- [x] **8. `/logprobs` command**:
