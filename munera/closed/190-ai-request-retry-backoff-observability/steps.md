...
- [x] Add focused coverage for streaming provider exceptions that carry retry headers in `ex-data`, and preserve those headers through the generic streaming exception path so provider-boundary retry honors `Retry-After` / rate-limit metadata for thrown failures as well as emitted `:error` events.
- [x] Preserve `:headers`, `:provider-error/headers`, `:http-status`, and `:status` through `psi.ai.streaming/exception->error-event`, with focused coverage for provider exceptions thrown inside the normal async streaming future path.
- [x] Add focused coverage for background streaming `:error` events that carry only `:provider-error/headers`, and update `make-provider-event-consumer` to forward normalized provider headers into the turn error path before retry metadata calculation.
