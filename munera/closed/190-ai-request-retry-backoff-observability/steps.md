...
- [ ] Add focused coverage for streaming provider exceptions that carry retry headers in `ex-data`, and preserve those headers through the generic streaming exception path so provider-boundary retry honors `Retry-After` / rate-limit metadata for thrown failures as well as emitted `:error` events.
