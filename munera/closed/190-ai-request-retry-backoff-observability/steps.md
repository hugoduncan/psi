...
- [ ] Add focused coverage for background streaming `:error` events that carry only `:provider-error/headers`, and update `make-provider-event-consumer` to forward normalized provider headers into the turn error path before retry metadata calculation.
