...
- [ ] Preserve `:headers`, `:provider-error/headers`, `:http-status`, and `:status` through `psi.ai.streaming/exception->error-event`, with focused coverage for provider exceptions thrown inside the normal async streaming future path.
