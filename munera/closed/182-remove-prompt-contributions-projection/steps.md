...
- [x] `bb test` — all green
- [x] Remove dead `prompt-contribution-schema` def from `session_state/model.clj` (line 66–76) — no longer referenced after `:prompt-contributions` schema entry removal
- [x] `bb test` — verify no regression after schema def removal
