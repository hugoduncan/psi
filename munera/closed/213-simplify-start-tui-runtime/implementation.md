...
2026-06-05 test-shaper review: no new actionable test-shape issue found. Applied `test-shaper` to the task artifacts plus `start-tui-runtime!`, `tui_wiring.clj`, `nrepl_runtime.clj`, and the public TUI startup/app-runtime tests. The current net is public-path, behaviour-focused, deterministic, and economical enough after TT1–TT17: it asserts observable state/outputs across provider lifetime, bootstrap/config/cwd/memory/session forwarding, nullable deterministic execution, command/action dispatch, active-input branches, OAuth handoff, UI widgets/chrome, terminal opts, and live nREPL focus publication. Verified `bb clojure:test:scry --namespace psi.app-runtime-tui-startup-test` → 14/91 green and `bb clojure:test:scry --namespace psi.app-runtime-test` → 32/128 green. No unchecked `steps.md` item added.

PASS_STATUS: REVIEW_COMPLETE

2026-06-05 docs review: no new actionable documentation issue found. Applied `review-task-docs` to the task artifacts plus the referenced implementation, tests, `README.md`, `doc/`, and `CHANGELOG.md`. The primary task remains a behaviour-preserving `start-tui-runtime!` refactor, which does not require user-facing docs. The only accepted user-visible behaviour change is the TT11 TUI idle queued-follow-up fix; `CHANGELOG.md` already has an Unreleased `Fixed` entry accurately describing that queued follow-up input while idle is now stored instead of dropped. `README.md` and `doc/tui.md` contain no stale or contradictory queued-follow-up example/claim, and no command/flag/path documentation changed. No unchecked `steps.md` follow-up added.
