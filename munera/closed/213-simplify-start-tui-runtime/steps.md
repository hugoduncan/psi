...
- [x] Compare `/tmp/after-local.json` with `munera/closed/213-simplify-start-tui-runtime/before-local.json` using the A2 logical-key reconciliation rule and confirm metric-derived touched-set total burden strictly decreases.
...
- [x] Run `bb gordian gate --baseline munera/closed/213-simplify-start-tui-runtime/before-diagnose.edn --fail-on new-cycles,new-high-findings --max-new-medium-findings 0` from the worktree root and confirm exit code `0`.
...
- [x] TT16 Add a public-path characterization test for `start-tui-runtime!` TUI terminal mode option forwarding: capture public TUI opts and assert `:alt-screen false` is present, so dropping the option and falling back to `psi.tui.app/start!`'s default true alt-screen behaviour fails through public startup.
- [x] TT17 Add a public-path characterization test for `start-tui-runtime!` nREPL active-session focus publication: after startup and a real public navigation/focus change, assert `psi.app-runtime.nrepl-runtime/active-session-id-in-session-state` reads the updated TUI focus from `app-runtime/session-state`, so dropping or publishing a stale `:tui-focus*` remains covered outside the local TUI callback closures.
