...
- [x] Preserve Git porcelain XY slots when classifying `git/status` (or use an untrimmed status command) and add coverage that a worktree-only tracked-file modification reports `:modified`, not `:staged`.
