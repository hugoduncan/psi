...
## Task-test-review pass 10 (test-shaper skill)

- added 1 follow-up step: `assert-recovery-turn-succeeded!` asserts the recovered text via `(str/includes? (pr-str content) "recovered")` — matching the serialized content vector rather than the extracted observable text value (`behavior_focused`); would also pass if `"recovered"` appeared elsewhere in the serialization and is brittle to content-shape changes. Non-compliance note: the task is closed (`munera/closed/`), not open as the review request assumed — step/note appended to the closed task in place.

- addressed 1 test-shaper (pass 10) follow-up: added `assistant-message-text` helper (`(->> content (keep :text) str/join)`) and switched `assert-recovery-turn-succeeded!` from `pr-str`-serialized-match to value-match on the extracted recovered text. Closes the `behavior_focused` gap (no longer passes on `"recovered"` in the print form outside the message text; not brittle to content-shape reshaping). `:turn.outcome/stop`/`:stop` checks unchanged. Non-vacuity verified (stub recovery delta → `NOTRECOVERED` fails both retry-footer tests). `bb test --focus psi.rpc-prompt-test` green (60 assertions, 3 tests); `clj-kondo` clean.
...
- addressed 1 pass-12 review step: retargeted the three stale-terminology docstrings (`retry-after-seconds`, `remaining-fragment`, `retry-footer-session-context!`) from "the driver" to the stub's `:error` stream event (`error-event`), matching the pass-5 wording. Docstring-only; `bb test --focus psi.rpc-prompt-test` green (47 assertions, 2 tests); `clj-kondo` clean.
