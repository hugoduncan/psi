...
- [ ] Add error-path e2e test for AC2: drive `run-tool-call!` through the `emit-tool-lifecycle!` bridge with `execute-tool-runtime-in!` returning `:is-error true`, register the metrics extension on the session ctx, and assert `@metrics-ext/store` accumulates a `:tools "…" :errors` entry — closes the split-coverage gap where the error-counter increment is only tested via direct `fire-event` (bridge bypassed) and the bridge's `:is-error` propagation is only asserted on the success path.
