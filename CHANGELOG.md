...
- New `reduce-incidental-complexity` workflow: autonomously simplifies one aspect of the system per run by targeting *incidental* complexity (high `gordian local` comprehension burden against low/moderate cyclomatic complexity). It selects the single highest `gap = lcc-total / max(cc, 1)` unit via the new `incidental-complexity-finder` skill, opens a constrained two-phase behaviour-preserving refactor task in an isolated worktree, and drives it through the full `task-lifecycle`. Stops early with no worktree/task when nothing qualifies; ends on the local worktree branch without pushing or opening a PR. Invokable via `/delegate reduce-incidental-complexity`.
...
### Fixed
- Active provider retry backoff now publishes `footer/updated` while retry state is active and again when it clears, so Emacs receives the existing app-runtime retry footer text (for example `retry in Ns`) without requiring a manual refresh.
