...
(ert-deftest psi-idle-resume-parity-no-arg-queries-sessions-and-selects-canonical-path ()
  (with-temp-buffer
    (psi-emacs-mode)
    (setq-local psi-emacs--state (psi-emacs--initialize-state (psi-test--spawn-long-lived-process)))
    (unwind-protect
        (let ((rpc-calls nil)
          ...
          (should (equal '("/tmp/sessions/a.ndedn") selected-paths)))
      (when (process-live-p (psi-emacs-state-process psi-emacs--state))
        (delete-process (psi-emacs-state-process psi-emacs--state))))))
...
(ert-deftest psi-idle-resume-parity-explicit-path-routes-to-path-handler-and-bypasses-prompt ()
  (with-temp-buffer
    (psi-emacs-mode)
    (setq-local psi-emacs--state (psi-emacs--initialize-state (psi-test--spawn-long-lived-process)))
    (unwind-protect
                      ...
                      ((symbol-function 'psi-emacs--handle-idle-resume-parity-explicit-path)
                       (lambda (_state session-path)
                         ...
                         (psi-emacs--append-assistant-message
                          (format "parity-path=%s" session-path))))
                      ((symbol-value 'psi-emacs--send-request-function)
                       (lambda (_state op params &optional _callback)
                         (push (list op params) rpc-calls))))
              (psi-emacs-send-from-buffer nil)))
          (should (equal '() no-arg-calls))
          (should (equal '("sessions/foo/bar") path-calls))
          (should (string-match-p "Assistant: parity-path=sessions/foo/bar" (buffer-string)))
          (should (equal '() rpc-calls)))
      (when (process-live-p (psi-emacs-state-process psi-emacs--state))
        (delete-process (psi-emacs-state-process psi-emacs--state))))))
