...
(defn- create-child-attempt-session
  [_ctx _parent-session-id opts]
  (child-attempt-session opts))
...
(defn- with-nullable-runtime
  [ctx {:keys [assistant-text judge-result]}]
  (test-support/with-workflow-execution-adapter-overrides
    (assoc ctx
           :workflow-create-step-attempt-session-fn create-child-attempt-session
           :workflow-execute-actor-turn-fn
           (fn [_ctx sid _prompt & _]
             {:status :ok
              :session-id sid
              ...
              :execution-result {}}))
    {:execute-judge! (fn [& _] judge-result)}))
  ...
  (let [[ctx0 session-id] (create-session-context)
        ...
        ctx (test-support/with-workflow-execution-adapter-overrides
              (assoc ctx0
                     ...
                     (fn [ctx _parent-session-id opts]
                                ...
                                (-> current-state
                                    ...
                                    (assoc-in [:workflows :runs "run-delegate-parent-race" :terminal-outcome]
                                              {:outcome :cancelled
                                               ...
                                               :step-id "delegate"})))))
                     (workflow-runtime/create-run state opts)))
...
(deftest judge-result-write-is-cancellation-safe-test
  ...
  (let [[ctx session-id] (create-session-context)
        ctx (with-nullable-runtime
              (assoc ctx
                     :before-workflow-live-state-update-fn
                     (fn [ctx {:keys [run-id kind]}]
                       (when (= :judge-record kind)
                         (swap! (:state* ctx)
                                (fn [current-state]
                                  (-> current-state
                                      (assoc-in [:workflows :runs run-id :status] :cancelled)
                                  ...
                                  (-> current-state
                                      ...
                                      (assoc-in [:workflows :runs run-id :terminal-outcome]
                                                {:outcome :cancelled
                                                 ...
                                                 :step-id "review"})))))))]
      ((:actions-fn (runtime/create-workflow-context ctx session-id "run-iteration-exhausted-cancel-race"))
       :iteration/exhausted {:step-id "review"}))
