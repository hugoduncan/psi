...
(deftest execute-judge-start-commit-to-call-race-is-cancellation-safe-test
  ...
  ;; successful judge turn :started commit but before the judge prompt adapter
  ;; call must still prevent the ordinary judge turn from starting.
  (let [prompt-calls* (atom 0)
        ctx {workflow-execution-adapter/adapter-key
             (workflow-execution-adapter/create
                 ...
                 (swap! (:state* ctx)
                        (fn [state]
                          (-> state
                              ...
                              (assoc-in [:workflows :runs workflow-run-id :terminal-outcome]
                                        {:outcome :cancelled
                                         :reason "judge start-commit race"
                                         :step-id workflow-step-id}))))))}
        initial-state (let [[s _ _] (workflow-runtime/create-run
                                     {}
                                     {:definition {:steps [{:name "review"
                                                            :type :session}]}
                                      :run-id "run-judge-start-commit-race"})]
                        (-> s
                            (assoc-in [:workflows :runs "run-judge-start-commit-race" :current-step-id] "review")
                            (assoc-in [:workflows :runs "run-judge-start-commit-race" :step-runs "review" :attempts]
                                      [{:attempt-id "attempt-review"
                                        :status :succeeded
                                        :execution-session-id "actor-review"}])))
        stopped? #(let [run (get-in @(:state* ctx)
...
(deftest execute-judge-call-begin-to-call-race-is-cancellation-safe-test
  ...
  ;; successful judge call-begin but before the prompt adapter call must still
  ;; prevent the ordinary judge turn from starting.
  (let [prompt-calls* (atom 0)
        ctx {workflow-execution-adapter/adapter-key
             (workflow-execution-adapter/create
              {:create-child-session! (fn [_ctx _parent opts]
                 ...
                 (swap! (:state* ctx)
                        (fn [state]
                          (-> state
                              ...
                              (assoc-in [:workflows :runs workflow-run-id :terminal-outcome]
                                        {:outcome :cancelled
                                         :reason "judge call-begin race"
                                         :step-id workflow-step-id}))))))}
        initial-state (let [[s _ _] (workflow-runtime/create-run
                                     {}
                                     {:definition {:steps [{:name "review"
                                                            :type :session}]}
                                      :run-id "run-judge-call-begin-race"})]
                        (-> s
                            (assoc-in [:workflows :runs "run-judge-call-begin-race" :current-step-id] "review")
                            (assoc-in [:workflows :runs "run-judge-call-begin-race" :step-runs "review" :attempts]
                                      [{:attempt-id "attempt-review"
                                        :status :succeeded
                                        :execution-session-id "actor-review"}])))
        stopped? #(let [run (get-in @(:state* ctx)
...
(deftest execute-judge-call-commit-to-call-race-is-cancellation-safe-test
  ...
  ;; successful judge call commit but before prompt adapter entry must still
  ;; prevent ordinary judge work from starting.
  (let [prompt-calls* (atom 0)
        ctx {workflow-execution-adapter/adapter-key
             (workflow-execution-adapter/create
              {:create-child-session! (fn [_ctx _parent opts]
                 ...
                 (swap! (:state* ctx)
                        (fn [state]
                          (-> state
                              ...
                              (assoc-in [:workflows :runs workflow-run-id :terminal-outcome]
                                        {:outcome :cancelled
                                         :reason "judge call-commit race"
                                         :step-id workflow-step-id}))))))}
        initial-state (let [[s _ _] (workflow-runtime/create-run
                                     {}
                                     {:definition {:steps [{:name "review"
                                                            :type :session}]}
                                      :run-id "run-judge-call-commit-race"})]
                        (-> s
                            (assoc-in [:workflows :runs "run-judge-call-commit-race" :current-step-id] "review")
                            (assoc-in [:workflows :runs "run-judge-call-commit-race" :step-runs "review" :attempts]
                                      [{:attempt-id "attempt-review"
                                        :status :succeeded
                                        :execution-session-id "actor-review"}])))
        stopped? #(let [run (get-in @(:state* ctx)
