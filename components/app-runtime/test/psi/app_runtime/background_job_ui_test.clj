...
(deftest refresh-background-jobs-ui-clears-stale-widget-and-statuses-test
  (let [[ctx session-id] (make-ctx)
        _ (session/dispatch-in! ctx :session/ui-set-widget {:extension-id "psi-background-jobs"
                                                            :widget-id "background-jobs"
                                                            :placement :below-editor
                                                            :content ["stale"]}
                                {:origin :test})
        _ (session/dispatch-in! ctx :session/ui-set-status {:extension-id "psi-background-jobs/job-old"
