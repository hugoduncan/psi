...
(def ^:private prose-graph-discovery
  (str "Capability graph (EQL discovery):\n"
       ...
       "- Reload code:\n"
       "  - first discover the running runtime source root when reload targeting is unclear: psi-tool(action: \"query\", query: \"[:psi.runtime/source-root :psi.agent-session/worktree-path]\", entity: \"{:psi.agent-session/session-id \\\"sid\\\"}\")\n"
       ...
       "  - if session worktree-path and runtime source-root differ, use runtime source-root for reload targeting\n"
