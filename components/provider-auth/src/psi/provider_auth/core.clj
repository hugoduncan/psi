...
(defn provider-api-key
  "Resolve a provider-scoped API key using shared precedence:
   ...
   `custom?` is the resolved model's `:custom?` origin tag.
   ...
   spec (literal or \"env:VAR\") — the transports'
...
(defn provider-request-options
  "Return provider-scoped request options derived from model-registry auth.
   ...
   provider name. The lookup is gated on the resolved model's `:custom?`
   origin tag so a built-in same-named model never inherits a custom
   provider's `:no-auth-header` or custom headers. Absent/nil is treated as
   built-in and returns nil: registry-derived options are applied only when
   the model is proven custom."
  ([provider]
