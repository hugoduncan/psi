     ...
     (let [steps (get-in definitions ["task-lifecycle-in-worktree" :steps])
       ...
       (testing "summary prompt reports the positive-path lifecycle terminal contract (TR10)"
         (let [summary-text (step-template-text summary-step)]
           ...
           (is (.contains summary-text "completed cleanly (design → plan → implement → review, then extract knowledge when review completed)")
               "summary reports whether the task-lifecycle run completed cleanly through the gated extraction phase")
           (is (.contains summary-text "knowledge-extraction result preserved by the inner `task-lifecycle` yield when extraction ran")
               "summary reports the extraction result preserved by the inner lifecycle yield")
           (is (.contains summary-text "review outcome that caused extraction to be skipped")
               "summary names the non-review-complete skip case surfaced to wrapper callers")
